...
import { adminUser, wrongUser, baseUrl } from './testData';


describe('Waterusages controller', () => {
    describe('GET /api/v1/water-usage', () => {
      
      it('responds with error token not found and statusCode 401', async () => {
      ...
      it('responds with error token not valid and statusCode 401', async () => {
    ...
    describe('POST /api/v1/water-usage/', () => {
      ...
      it('responds with  status code 400', async () => {
        const login = await request(baseUrl).post('api/v1/login').send(adminUser);
        const token = login.body.token;
        const response = await request(baseUrl).post('api/v1/water-usage/').set('Authorization', `Bearer ${token}`).send(withoutAmount);
      ...
      it('responds with  status code 400', async () => {
        const login = await request(baseUrl).post('api/v1/login').send(adminUser);
        const token = login.body.token;
        const response = await request(baseUrl).post('api/v1/water-usage/').set('Authorization', `Bearer ${token}`).send(withoutConsumptionTime);
      ...
      it('responds with  status code 400', async () => {
        const login = await request(baseUrl).post('api/v1/login').send(adminUser);
        const token = login.body.token;
        const response = await request(baseUrl).post('api/v1/water-usage/').set('Authorization', `Bearer ${token}`).send(withoutWmID);
      ...
      it('responds with  status code 401', async () => {
        const response = await request(baseUrl).post('api/v1/water-usage/').send(newUsage);
